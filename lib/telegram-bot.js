...
function createBotInstance(options = {}) {
    return new node_telegram_bot_api_1.default(botToken, options);
}
class TelegramBotService {
    ...
    async handleStartCommand(chatId, telegramUser) {
        try {
            if (!isSupabaseConfigured) {
                await this.bot.sendMessage(chatId, '❌ Bot is not properly configured. Please contact the administrator.');
                return;
            }
            ...
            const { data: existingUser } = await supabase
                ...
                .single();
            if (existingUser) {
                await this.bot.sendMessage(chatId, '✅ You are already linked to your account! You can now use the bot to manage your expenses.');
                return;
            }
            ...
            if (insertError) {
                ...
                await this.bot.sendMessage(chatId, '❌ Sorry, there was an error generating your authentication link. Please try again.');
                return;
            }
            ...
            await this.bot.sendMessage(chatId, `🔐 Welcome to Baxter Expense Manager! [TEST-UNIQUE-STRING-123]
...
⚠️ Important: This link will expire in 10 minutes for security reasons. You can sign in or sign up after clicking the link.`);
    ...
    async handleAuthCallback(query) {
        try {
            ...
            await this.bot.sendMessage(chatId, '🎉 Successfully linked! Your Telegram account is now connected to your Baxter account. You can now use the bot to manage your expenses.');
        }
        catch (error) {
    ...
    async handleTextMessage(msg) {
        try {
            ...
            const { data: linkedUser } = await supabase
                ...
                .single();
            if (!linkedUser) {
                await this.bot.sendMessage(chatId, '❌ Please link your account first by sending /start');
                return;
            }
    ...
    async linkTelegramUser(telegramId, supabaseUserId) {
        try {
            const supabase = createSupabaseClient();
    ...
    async getSupabaseUserId(telegramId) {
        try {
            const supabase = createSupabaseClient();
